...
float intersect_box(BOX box, RAY ray) {
  ...
  if (ray.direction.x != 0) {
    float s = (box.sx / 2 - xadj) / ray.direction.x;
    if (s > 0 && s < ss) {
      yhit = abs(yadj + s * ray.direction.y);
      zhit = abs(zadj + s * ray.direction.z);

      if ((yhit < box.sy / 2) && (zhit < box.sz / 2)) {
        ss = s;
      }
    }

    s = (-box.sx / 2 - xadj) / ray.direction.x;
    if ((s > 0) && (s < ss)) {
      yhit = abs(yadj + s * ray.direction.y);
      zhit = abs(zadj + s * ray.direction.z);
      if ((yhit < box.sy / 2) && (zhit < box.sz / 2)) {
  ...
  if (ray.direction.y != 0) {
    float s = (box.sy / 2 - yadj) / ray.direction.y;
    if (s > 0 && s < ss) {
      xhit = abs(xadj + s * ray.direction.x);
      zhit = abs(zadj + s * ray.direction.z);

      if ((xhit < box.sx / 2) && (zhit < box.sz / 2)) {
        ss = s;
      }
    }

    s = (-box.sy / 2 - yadj) / ray.direction.y;
    if ((s > 0) && (s < ss)) {
      xhit = abs(xadj + s * ray.direction.x);
      zhit = abs(zadj + s * ray.direction.z);
      if ((xhit < box.sx / 2) && (zhit < box.sz / 2)) {
  ...
  if (ray.direction.z != 0) {
    float s = (box.sz / 2 - zadj) / ray.direction.z;
    if (s > 0 && s < ss) {
      xhit = abs(xadj + s * ray.direction.x);
      yhit = abs(yadj + s * ray.direction.y);

      if ((xhit < box.sx / 2) && (yhit < box.sy / 2)) {
        ss = s;
      }
    }

    s = (-box.sz / 2 - zadj) / ray.direction.z;
    if ((s > 0) && (s < ss)) {
      xhit = abs(xadj + s * ray.direction.x);
      yhit = abs(yadj + s * ray.direction.y);
      if ((xhit < box.sx / 2) && (yhit < box.sy / 2)) {
...
vec3 normal_box(BOX box, vec3 intersection) {
  ...
  diff = abs((box.center.x + box.sx / 2) - intersection.x);
  if (ss > diff) {
    ss = diff;
    face = 0;
  }
  ...
  diff = abs((box.center.y + box.sy / 2) - intersection.y);
  if (ss > diff) {
    ss = diff;
    face = 2;
  }
  ...
  diff = abs((box.center.z + box.sz / 2) - intersection.z);
  if (ss > diff) {
    ss = diff;
    face = 4;
  }
...
vec3 phong(MATERIAL material, SCENE scene, vec3 N, vec3 intersection) {
  vec3 color = vec3(0,0,0); //final color
  ...
  for (int i = 0; i < scene.nr_lights; i++) {
    ...
    if (diff > 0) {
     ...
     color += material.kd * light.ld * diff;
      ...
      float spec = dot(R,L);
      if (spec > 0) {
        spec = max(0, pow(spec, material.ns));
...
RAY shoot_ray(CAMERA camera, vec3 pos) {
  ...
  float zvp = 1/(tan(radians(camera.fov/2)));
...
SCENE create_scene_1() {
  ...
  scene.camera.position = vec3(2,0,2);
  scene.camera.view_up = vec3(0,1,0);
  scene.camera.look_at = vec3(0,0,0);
  scene.camera.fov = 35;
 
  scene.nr_lights = 1;
  scene.lights[0].position = vec3(10,10,10);
  scene.lights[0].la = vec3(1,1,1);
  scene.lights[0].ld = vec3(1,1,1);
  scene.lights[0].ls = vec3(1,1,1);

  scene.nr_spheres = 3;
  scene.spheres[0].center = vec3(0,0,0);
  ...
  scene.spheres[0].material.ns = 128;
  scene.spheres[0].material.reflt = 0.5f;

  scene.spheres[1].center = vec3(1.5f,0,0.5f);
  ...
  scene.spheres[1].material.ns = 128;
  scene.spheres[1].material.reflt = 0.15f;

  scene.spheres[2].center = vec3(-1.15f,-0.75f,0.5f);
  ...
  scene.spheres[2].material.ns = 128;
...
SCENE create_scene_2() {
  ...
  scene.camera.position = vec3(-10, 15, 30);
  scene.camera.view_up = vec3(0,1,0);
  scene.camera.look_at = vec3(0,0,0);
  scene.camera.fov = 35;
 
  scene.nr_lights = 1;
  scene.lights[0].position = vec3(10, 25, 25);
  scene.lights[0].la = vec3(1,1,1);
  scene.lights[0].ld = vec3(1,1,1);
  scene.lights[0].ls = vec3(1,1,1);
  
  scene.nr_boxes = 1;
  scene.boxes[0].center = vec3(0, 0, 0);
  scene.boxes[0].sx = 15;
  scene.boxes[0].sy = 1;
  scene.boxes[0].sz = 15;
  ...
  scene.boxes[0].material.ns = 64;
  scene.boxes[0].material.reflt = 0.15f;
  
  scene.nr_spheres = 9;
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      scene.spheres[i*3+j].center = vec3(-5 + j * 5, 1.75f, -5 + i * 5);
      ...
      if (i == 0) {
        ...
        scene.spheres[i*3+j].material.ns = 64;
...
void main(void)
{
  SCENE scene = create_scene_2();
  fragColor = vec4(ray_casting(scene, v_position), 1);
}
